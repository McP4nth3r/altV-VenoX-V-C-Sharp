    ...
    public class Round
    {
        ...
        private void GetNewMap()
        {
            try
            {
                ...
                int randomMap = random.Next(0, global::VenoX.Core.Tactics.maps.Main.TacticMaps.Count); // Count Spawnpoints
                CurrentMap = global::VenoX.Core.Tactics.maps.Main.TacticMaps[randomMap];
                if (LastMap == CurrentMap) { GetNewMap(); return; }
                LastMap = CurrentMap;
            }
            catch { CurrentMap = global::VenoX.Core.Tactics.maps.Main.TacticMaps[1]; }
        }
        ...
        public async void SyncEndTacticRound(string text)
        {
            try
            {
                foreach (var players in Main.TacticsPlayers.ToList().Where(players => players is not null && players.Exists))
                {
