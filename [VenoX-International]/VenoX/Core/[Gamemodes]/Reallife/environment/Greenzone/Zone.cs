    ...
    public class Zone : IScript
    {
        ...
        public static bool OnPlayerEnterColShapeModel(ColShapeModel shape, VnXPlayer player)
        {
            try
            {
                ...
                Task.Run(async () =>
                {
                    string text1 = await Main.GetTranslatedTextAsync((Main.Languages) player.Language, "You have entered a NO-DM zone!");
                    string text2 = await Main.GetTranslatedTextAsync((Main.Languages) player.Language, "Any deathmatch is prohibited!");
                    string text3 = await Main.GetTranslatedTextAsync((Main.Languages) player.Language, "Exception : state factions.");
