...
alt.onServer('Zombies:Sync', (state, playerCount = 0) => {
    try {
        IsSyncer = state;
        if (IsSyncer) {
...
alt.onServer('Zombies:SpawnKI', (Id, RandomSkinUID, Hash, Position, Target) => {
...
alt.onServer('Zombies:SetHealth', (Id, Health) => {
    if (!Zombies[parseInt(Id)]) return;
    game.setEntityHealth(Zombies[parseInt(Id)].Entity, parseInt(Health), 0);
});

alt.onServer('Zombies:UpdatePositionAndRotation', (Id, PosX, PosY, PosZ, RotX, RotY, RotZ) => {
    if (!Zombies[parseInt(Id)]) return;
    game.setEntityCoords(Zombies[parseInt(Id)].Entity, PosX, PosY, PosZ, false, false, false, false);
...
alt.onServer('Zombies:MoveToTarget', (ID, Hash, Position, Rotation, TargetEntity, move) => {
    if (!Zombies[parseInt(ID)]) {
        SpawnZombie(parseInt(ID), Hash, Position, TargetEntity, null);
        return;
    }
    ...
    game.setEntityCoords(Zombies[parseInt(ID)].Entity, Position.x, Position.y, Position.z, false, false, false, false);
    game.setEntityRotation(Zombies[parseInt(ID)].Entity, Rotation.x, Rotation.y, Rotation.z, 2, true);
});

alt.onServer(
    'Zombies:LoadEntityClassClothes',
    (
        ID,
        Slot1Drawable,
        Slot1Texture,
...
alt.onServer(
    'Zombies:LoadEntityClassAccessories',
    (
        ID,
        Slot1Drawable,
        Slot1Texture,
...
alt.onServer('Zombies:LoadEntityClass', (ID, FaceFeatures, HeadBlendData, HeadOverlays) => {
...
function DeleteZombieById(ID: number) {
...
function CheckZombieHealths() {
    for (var Id in Zombies) {
        if (Zombies[Id].Entity != null && !Zombies[Id].IsDead) {
            if (game.hasEntityBeenDamagedByEntity(Zombies[Id].Entity, alt.Player.local.scriptID, false)) {
...
function DrawNametags() {
    for (var Id in Zombies) {
        if (Zombies[Id].Entity != null) {
            let playerPos2 = game.getEntityCoords(Zombies[Id].Entity, true);
            ...
            if (distance <= 30)
                DrawText(
                    'Zombie ~r~[' + Zombies[Id].Id + ']\n~b~ Move : ~w~' + Zombies[Id].MoveTime + '\nCDate : ' + CurrentDateTime,
                    [screenPos[1], screenPos[2] - 0.035],
                    [0.55, 0.55],
                    4,
                    [255, 255, 255, 255],
                    true,
...
function SetZombieCorrectSkin(zombieEntity: number, RandomSkinUID: number) {
    ...
    game.setPedHeadBlendData(zombieEntity, headblends[0], headblends[1], 0, headblends[2], headblends[5], 0, headblends[3], headblends[4], 0, false);
    ...
    game.setPedComponentVariation(zombieEntity, 1, ZombieClothes[RandomSkinUID].Slot1Drawable, ZombieClothes[RandomSkinUID].Slot1Texture, 0);
    game.setPedComponentVariation(zombieEntity, 2, ZombieClothes[RandomSkinUID].Slot2Drawable, ZombieClothes[RandomSkinUID].Slot2Texture, 0);
    game.setPedComponentVariation(zombieEntity, 3, ZombieClothes[RandomSkinUID].Slot3Drawable, ZombieClothes[RandomSkinUID].Slot3Texture, 0);
    game.setPedComponentVariation(zombieEntity, 4, ZombieClothes[RandomSkinUID].Slot4Drawable, ZombieClothes[RandomSkinUID].Slot4Texture, 0);
    game.setPedComponentVariation(zombieEntity, 5, ZombieClothes[RandomSkinUID].Slot5Drawable, ZombieClothes[RandomSkinUID].Slot5Texture, 0);
    game.setPedComponentVariation(zombieEntity, 6, ZombieClothes[RandomSkinUID].Slot6Drawable, ZombieClothes[RandomSkinUID].Slot6Texture, 0);
    game.setPedComponentVariation(zombieEntity, 7, ZombieClothes[RandomSkinUID].Slot7Drawable, ZombieClothes[RandomSkinUID].Slot7Texture, 0);
    game.setPedComponentVariation(zombieEntity, 8, ZombieClothes[RandomSkinUID].Slot8Drawable, ZombieClothes[RandomSkinUID].Slot8Texture, 0);
    game.setPedComponentVariation(zombieEntity, 9, ZombieClothes[RandomSkinUID].Slot9Drawable, ZombieClothes[RandomSkinUID].Slot9Texture, 0);
    game.setPedComponentVariation(zombieEntity, 10, ZombieClothes[RandomSkinUID].Slot10Drawable, ZombieClothes[RandomSkinUID].Slot10Texture, 0);
    game.setPedComponentVariation(zombieEntity, 11, ZombieClothes[RandomSkinUID].Slot11Drawable, ZombieClothes[RandomSkinUID].Slot11Texture, 0);
...
function SetZombieAttributes(zombie: number) {
    ...
    game.setPedFleeAttributes(zombie, 0, false);
    game.setPedCombatAttributes(zombie, 16, true);
    game.setPedCombatAttributes(zombie, 17, false);
    game.setPedCombatAttributes(zombie, 46, true);
    game.setPedCombatAttributes(zombie, 1424, false);
    game.setPedCombatAttributes(zombie, 5, true);
    ...
    if (game.hasAnimSetLoaded('move_m@drunk@verydrunk')) {
        game.setPedMovementClipset(zombie, 'move_m@drunk@verydrunk', 0);
    }
...
/*
function loadModel(dict: string | number) {
    dict = parseInt(dict);
...
*/
function SpawnZombie(Id: number, RandomSkinUID: number, Hash: string | number, Position: { x: number; y: number; z: number }, Target: any) {
    if (Zombies[Id]) return;
    Hash = alt.hash('' + Hash);
...
function MoveZombieToTarget(ID: string, TargetEntity: any) {
    if (Zombies[ID].IsDead) return;
    if (Zombies[ID].Frozen) game.freezeEntityPosition(Zombies[ID].Entity, false);
    ...
    if (!game.isPedRunning(Zombies[ID].Entity)) {
        ...
        game.playPain(Zombies[ID].Entity, 8, 0, 0);
    }
}

alt.everyTick(() => {
    //DrawNametags();
    for (var Id in Zombies) if (Zombies[Id].MoveTime && Zombies[Id].MoveTime <= CurrentDateTime) MoveZombieToTarget(Id, Zombies[Id].TargetEntity);
});
