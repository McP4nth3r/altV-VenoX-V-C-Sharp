...
function createX(type, tag = "normal") {
	...
	piemenu.sliceTransformFunction = sliceTransform().ScaleTitleTransform;
	piemenu.selectedNavItemIndex = null;
	piemenu.slicePathFunction = slicePath().DonutSlice;
	piemenu.clickModeRotate = false;

	if (type == "veh") {
		piemenu.createWheel([icon.lock, icon.cross, icon.bolt,  icon.books]);
		piemenu.navItems[0].navSlice.mouseover(function () { setID(0, 9900); });
		piemenu.navItems[2].navSlice.mouseover(function () { setID(1, 9901); });	
		piemenu.navItems[1].navSlice.mouseover(function () { setID(2, 0); });
		piemenu.navItems[3].navSlice.mouseover(function () { setID(3, 9902); });
		$itemNames = ["Lock / Unlock",  "Engine on / Engine off", "Close", "Information"];
	}
	else if (type == "self") {
