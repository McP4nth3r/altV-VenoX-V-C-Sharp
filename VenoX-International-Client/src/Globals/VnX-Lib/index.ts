...
export function ShowCursor(bool: boolean) {
    try {
...
export function DrawText(msg: string, screenPos: number[], scale: number[], fontType: number, ColorRGB: number | number[], useOutline = true, useDropShadow = true, layer = 0, align = 0) {
    try {
        let hex = msg.match('{.*}');
        /*if (hex) {
...
export function CreateBlip(name: string, pos: any[], sprite: number, color: number, shortrange: boolean) {
    try {
...
export function CreatePed(PedName: string, Vector3Pos: { x: number; y: number; z: number }, rot = 0) {
    try {
        let PedHash = game.getHashKey(PedName);
        if (!game.hasModelLoaded(PedHash)) game.requestModel(PedHash);
...
export function frontOfPlayer(distance: number) {
    try {
...
export function vnxCreateCEF(Name: string, Path: string) {
    try {
        for (var Browser in cBrowserList) if (cBrowserList[Browser].Name == Name) return;
...
alt.onServer('Lib:DebugCEFBrowser', () => {
    for (var Browser in cBrowserList) alt.emitServer('CEF:Debug', cBrowserList[Browser].Name, cBrowserList[Browser].Path);
});

export function vnxDestroyCEF(Name: string) {
    try {
        for (var Browser in cBrowserList) {
            if (cBrowserList[Browser].Name == Name) {
                if (cBrowserList[Browser].CEF != null) cBrowserList[Browser].CEF.destroy();
...
export function CreateSideMenu() {
    ...
    SideMenu.on('sidemenu:purchaseItem', (selected: any, purchasedAmount: any) => {
