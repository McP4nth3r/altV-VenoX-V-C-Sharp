        ...
        public static void Ban_Player_TeleportCheat(IPlayer player)
        {
            try
            {
                if (player.Health != 0)
                {
                    ...
                    Reallife.Core.RageAPI.SendChatMessageToAll(RageAPI.GetHexColorcode(255,0,0) +player.Name + " wurde von [VenoX Anti-Cheat Shield] gekickt! Grund : # " + Banhash);
        ...
        public static void Ban_Player_SpeedCheat(IPlayer player, int distanz, string position_now, string position_last)
        {
            try
            {
                if (player.Health != 0)
                {
                    ...
                    logfile.WriteAntiCheatLogs("speed", "[ANTI-CHEAT][0x005] : " +player.Name + " | Distanz : " + distanz + " | Position Now : " + position_now + " | Position Last : " + position_last);
                }
            }
            catch { }
        }
