...
namespace VenoX.Core._Globals_.Inventory
{
    public class Inventory
    {
        ...
        public static WeaponModel GetWeaponModelByItemHash(string hash)
        {
            ...
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); return WeaponModel.Fist; }
        }

        private static readonly Dictionary<string, string> _getObjectHash = new Dictionary<string, string>
        {
        ...
        public static void CreateDroppedObject(VnXPlayer player, ItemModel item)
        {
            try
            {
                ...
                switch (item.Type)
                {
                    ...
                    default:
                        ConsoleHandling.OutputDebugString("obj not found : " + item.Hash);
                        break;
                }
                
                foreach (VnXPlayer nearby in player.NearbyPlayers.ToList())
                    _RootCore_.VenoX.TriggerClientEvent(nearby, "Inventory:DropObj", item.Id, obj, item.Hash + "[" + item.Id + "]\nPress ~b~'E' ~w~ to pickup.");

                _RootCore_.VenoX.TriggerClientEvent(player, "Inventory:DropObj", item.Id, obj, item.Hash + "[" + item.Id + "]\nPress ~b~'E' ~w~ to pickup.");
        ...
        public static void DeleteDroppedObject(VnXPlayer player, ItemModel item)
        {
            if (player is not null && player.Exists && item is not null)
                _RootCore_.VenoX.TriggerClientEvent(player, "Inventory:DeleteObj", item.Id);
        }
        private static readonly Dictionary<string, WeaponModel> _weaponHashes = new Dictionary<string, WeaponModel>
        {
