...
using VenoX.Core._Gamemodes_.SevenTowers.maps.main;
using VenoX.Core._Gamemodes_.SevenTowers.model;
using VenoX.Core._RootCore_;
using VenoX.Core._RootCore_.Models;
using VenoX.Core._RootCore_.Sync;
using VenoX.Debug;

namespace VenoX.Core._Gamemodes_.SevenTowers.lobby
{
    public class Main
    {
        ...
        public static int SeventowersDim = _Globals_.Initialize.SeventowersDimension;
        ...
        public static void PutPlayerSpectate(VnXPlayer player)
        {
            try
            {
                ...
                _RootCore_.VenoX.TriggerClientEvent(player, "SevenTowers:PutPlayerIntoSpectatorMode");
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
        ...
        public static void CreateNewHitMarker()
        {
            try
            {
                ...
                foreach (VnXPlayer players in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers)) Sync.ForceClientSyncUpdate(players);
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
        ...
        public static void StartNewRound()
        {
            try
            {
                ...
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers))
                {
                ...
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers))
                {
                    if (CurrentlyInRound.Count == 1)
                    {
                        foreach (VnXPlayer winner in CurrentlyInRound.ToList())
                        {
                            ...
                            {
                                if (Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers).Count > 1) winner.SevenTowers.Wins += 1;
                                CurrentWinner = winner.CharacterUsername;
                                TakePlayerFromRound(player);
                            }
                        }
                    }
                    string translatedText = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)player.Language, "gewinnt die Runde.");
                    _RootCore_.VenoX.TriggerClientEvent(player, "SevenTowers:ShowWinner", CurrentWinner, translatedText, 5000);
                }
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
        ...
        public static void SpawnPlayerInRound(VnXPlayer player)
        {
            try
            {
                ...
                {
                    if (player.SevenTowers.IsSpectator) { _RootCore_.VenoX.TriggerClientEvent(player, "SevenTowers:RemovePlayerFromSpectatorMode"); player.SevenTowers.IsSpectator = false; }
                    Random random = new Random();
        ...
        public static async void PutPlayerInRound(VnXPlayer player)
        {
            try
            {
                ...
                {
                    string joinedLobbytext = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)players.Language, " hat die SevenTowers Lobby betreten!");
                    players.SendChatMessage(RageApi.GetHexColorcode(0, 200, 255) + player.CharacterUsername + RageApi.GetHexColorcode(255, 255, 255) + " " + joinedLobbytext);
                }
                InitializePlayerData(player);
        ...
        public static void JoinedSevenTowers(VnXPlayer player)
        {
            try
            {
                ...
                if (_Globals_.Initialize.SevenTowersPlayers.Count <= 1) { StartNewRound(); return; }
        ...
        public static void OnUpdate()
        {
            try
            {
                if (_Globals_.Initialize.SevenTowersPlayers.Count <= 0) return;

                if (SeventowersRoundEnd <= DateTime.Now && SeventowersRoundIsRunning)
                {
                    foreach (VnXPlayer playerClass in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers)) TakePlayerFromRound(playerClass);
                    EndRound();
                }
                else switch (SeventowersRoundIsRunning)
                {
                    ...
                    {
                        foreach (VnXPlayer playerClass in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.SevenTowersPlayers).Where(playerClass => playerClass.Position.Z <= 0 && playerClass.SevenTowers.Spawned))
                            TakePlayerFromRound(playerClass);
                        break;
                    }
                }
