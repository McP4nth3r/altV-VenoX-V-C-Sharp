                    ...
                    if (factions.Allround.IsBadFaction(player))
                    {
                        if (player.IsInVehicle)
                        {
                            ...
                            if (vehClass.Reallife.ActionVehicle)
                            {
                                ...
                                foreach (VnXPlayer otherplayers in Enumerable.ToList<VnXPlayer>(Initialize.ReallifePlayers)) otherplayers.SendTranslatedChatMessage(RageApi.GetHexColorcode(200, 200, 200) + "[Illegal]: Der Kokaintruck wurde abgegeben!");
