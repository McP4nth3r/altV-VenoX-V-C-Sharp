...
namespace VenoX.Core._Gamemodes_.Reallife.fun.Aktionen.Shoprob
{
    public class Shoprob : IScript
    {
        ...
        public static void CreateShopRobPedsIPlayer(VnXPlayer player)
        {
            try
            {
                ...
                foreach (var shopCoord in ShopSkins)
                {
                    _RootCore_.VenoX.TriggerClientEvent(player, "ShopRob:CreateNPC", "s_m_m_ups_01", shopCoord.Key, shopCoord.Value);
                }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
        ...
        public static void OnPlayerEnterColShapeModel(ColShapeModel shape, VnXPlayer player)
        {
            try
            {
                ...
                {
                    _RootCore_.VenoX.TriggerClientEvent(player, "CreateShopWindow");
            ...
            try
            {
                if (player.VnxGetElementData<bool>(PlayerRobStarted) != true)
                {
                    if (player.VnxGetElementData<bool>(ShopId))
                    {
                        ...
                        {
                            Notification.DrawNotification(player, Notification.Types.Error, "Du bist in einer Staatsfraktion!");
                        }
                    }
                }
            }
        ...
        public static void OnUpdate()
        {
            try
            {
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.ReallifePlayers))
                {
