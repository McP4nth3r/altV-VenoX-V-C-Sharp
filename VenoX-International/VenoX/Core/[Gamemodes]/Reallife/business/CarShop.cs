    ...
    public class CarShop : IScript
    {
        ...
        public static bool OnPlayerEnterColShapeModel(ColShapeModel shape, VnXPlayer player)
        {
            try
            {
                ...
                _RootCore_.VenoX.TriggerClientEvent(player, "VehicleCatalog:Show");
                // Getting the speed for each IVehicle in the list
                foreach (CarShopVehicleModel carShopVehicle in carList)
                    _RootCore_.VenoX.TriggerClientEvent(player, "VehicleCatalog:Fill", carShopVehicle.Type, carShopVehicle.Model, carShopVehicle.Price, carList.Count);
        ...
        public static void TestVehicle(VnXPlayer player, string model, string firstColor, string secondColor)
        {
            try
            {
                foreach (_RootCore_.Models.VehicleModel vehicleClass in Enumerable.ToList<_RootCore_.Models.VehicleModel>(Initialize.AllVehicles))
                {
                    if (vehicleClass.IsTestVehicle && vehicleClass.Owner == player.CharacterUsername)
                        RageApi.DeleteVehicleThreadSafe(vehicleClass);
                }
                player.Dimension = (_Globals_.Initialize.ReallifeDimension + player.Language) + player.Id;
                ...
                _RootCore_.Models.VehicleModel vehClass = (_RootCore_.Models.VehicleModel)Alt.CreateVehicle(vehicleModel, new Vector3(-51.54087f, -1076.941f, 26.94754f), new Vector3(0, 0, 180f));
                vehClass.Name = "" + vehicleModel;
                vehClass.FirstColor = color1.R + "," + color1.G + "," + color1.B;
                vehClass.SecondColor = color2.R + "," + color2.G + "," + color2.B;
                vehClass.Owner = player.CharacterUsername;
        ...
        public static void BuyVehicle(VnXPlayer player, string model, string firstColor, string secondColor)
        {
            try
            {
                ...
                if (vehiclePrice > 0 && player.Reallife.Bank >= vehiclePrice)
                {
                    switch (carShop)
                    {
                        case 0:
                            ...
                            vehClass.Owner = player.CharacterUsername;
                            vehClass.Plate = player.CharacterUsername;
                            vehClass.Gas = 100;
                            vehClass.Kms = 0;
                            vehClass.Faction = 0;
                            vehClass.Frozen = false;
                ...
                {
                    Notification.DrawTranslatedNotification(player, _Globals_.Notification.Types.Error, "Du hast nicht genug Geld auf der Bank!");
                }
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }

        public static void OnPlayerLeaveVehicle(_RootCore_.Models.VehicleModel vehicle, VnXPlayer player)
        {
            try
            {
                if (vehicle.IsTestVehicle && vehicle.Owner == player.CharacterUsername)
                {
