...
namespace VenoX.Core._Gamemodes_.Reallife.gangwar
{
    public class Allround : IScript
    {
        ...
        public static void AtackGangwarArea(VnXPlayer player)
        {
            try
            {
                ...
                if (GangwarManager.CurrentArea == null)
                {
                    ...
                    foreach (GangwarArea area in GangwarManager.GangwarAreas.Where(area => area.Tk.Distance(player.Position) < GangwarManager.TkRange))
                    {
                            ...
                            if (player.Reallife.FactionRank >= GangwarManager.MinRankAttack)
                            {
                                ...
                                if (GangwarManager.GetFactionCount(area.IdOwner) >= GangwarManager.MinCountPlayer)
                                {
                                    ...
                                    if (area.IsAttackable())
                                    {
                                        ...
                                        Faction.CreateCustomFactionInformation(GangwarManager.CurrentArea.GetCurrentRound().AttackerId, RageApi.GetHexColorcode(0, 200, 0) + player.CharacterUsername + " hat einen Gangwar gegen " + Faction.GetFactionNameById(GangwarManager.CurrentArea.GetCurrentRound().DefenderId) + " gestartet!");
                                        return;
                                    }
                                ...
                                else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Die Verteidiger Fraktion haben haben nicht genug Spieler online!"); }
                            }
                            else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Du bist nicht befugt dieses Gebiet anzugreifen!"); }
                        }
                        else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Du kannst nicht deine eigene Fraktion angreifen!"); }
                    }
                }
                else
                {
                    ...
                    if (factions.Allround.IsBadFaction(player) && player.Reallife.Faction == GangwarManager.CurrentArea.GetCurrentRound().AttackerId)
                    {
                        ...
                        if ((GangwarManager.AttackerCountMore && attCount <= GangwarManager.GetFactionCount(GangwarManager.CurrentArea.IdOwner)) || (!GangwarManager.AttackerCountMore && attCount < GangwarManager.GetFactionCount(GangwarManager.CurrentArea.IdOwner)))
                        {
                            ...
                            Faction.CreateCustomFactionInformation(GangwarManager.CurrentArea.GetCurrentRound().AttackerId, RageApi.GetHexColorcode(0, 200, 0) + player.CharacterUsername + " nimmt nun am Gangwar teil.");
                        }
                        else { Notification.DrawNotification(player, Notification.Types.Error, "Ihr seid schon genug Angreifer!"); }
                    }
                    else { Notification.DrawNotification(player, Notification.Types.Error, "Du bist kein Angreifer oder in einer Bösen Fraktion!"); }
                }
            }
                ...
                if (factions.Allround.IsBadFaction(player) && player.Reallife.Faction == GangwarManager.CurrentArea.GetCurrentRound().DefenderId)
                {
                    ...
                    if (GangwarManager.CurrentArea.Position.Distance(player.Position) > GangwarManager.MinDist)
                    {
                        ...
                        if (defCount < allowedDefCount)
                        {
                            ...
                            {
                                GangwarManager.CurrentArea.AddPlayer(player);
                                Faction.CreateCustomFactionInformation(GangwarManager.CurrentArea.GetCurrentRound().DefenderId, RageApi.GetHexColorcode(0, 200, 0) + player.CharacterUsername + " nimmt nun am Gangwar teil.");
                                Faction.CreateCustomFactionInformation(GangwarManager.CurrentArea.GetCurrentRound().AttackerId, RageApi.GetHexColorcode(150, 0, 0) + player.CharacterUsername + " nimmt nun am Gangwar teil.");
                            }
                            else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Du bist leider zu spaet zum GW!"); }
                        }
                        else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Ihr seid schon zu viele"); }
                    }
                    else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Du musst weiter weg vom Gebiet sein!"); }
                }
                else { Notification.DrawTranslatedNotification(player, Notification.Types.Error, "Du bist kein Verteidiger oder in einer Bösen Fraktion!"); }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
        ...
        public static bool OnPlayerEnterColShapeModel(ColShapeModel shape, VnXPlayer player)
        {
            try
            {
                foreach (GangwarArea area in GangwarManager.GangwarAreas)
                {
                    ...
                    {
                        Notification.DrawTranslatedNotification(player, Notification.Types.Warning, "Du hast ein Ganggebiet betreten!");
                        return true;
                    }
        ...
        public static void OnPlayerDisconnected(VnXPlayer player, string type, string reason)
        {
            ...
            {
                if (GangwarManager.CurrentArea == null) return;
                GangwarRound.PlayerEntry quiter = GangwarManager.CurrentArea.GetCurrentRound().GetPlayerEntry(player);
                if (quiter != null)
                {
        ...
        public static void OnPlayerDeath(VnXPlayer player, VnXPlayer killer, uint reason)
        {
            ...
            {
                if (GangwarManager.CurrentArea == null) return;
                GangwarRound.PlayerEntry playerEntry = GangwarManager.CurrentArea.GetCurrentRound().GetPlayerEntry(player);
                GangwarRound.PlayerEntry killerEntry = GangwarManager.CurrentArea.GetCurrentRound().GetPlayerEntry(killer);
