...
namespace VenoX.Core._Gamemodes_.Reallife.gangwar.v2
{
    public class GangwarRound
    {
        ...
        private readonly DateTime _preparingCd;
        private readonly DateTime _maxTime;
        ...
        public void Decided(int winnerId, int loserId, string state)
        {
            try
            {
                ...
                ResetPlayer();
                _RootCore_.VenoX.TriggerEventForAll("StopCurrentGangwar", _gangwarArea.Name);
        ...
        public string GetFactionInfo(int facId)
        {
            try
            {
                int max = 0;
                int alive = 0;
                foreach (PlayerEntry entry in PlayerList.ToList())
                {
                    if (!entry.IsLeft)
                    {
        ...
        public void ProcessDamage(VnXPlayer source, VnXPlayer player, float damage)
        {
            try
            {
                PlayerEntry sourceEntry = GetPlayerEntry(source);
                PlayerEntry playerEntry = GetPlayerEntry(player);
            ...
            try
            {
                ...
                if (CurrentState == RoundStates.Preparing)
                {
                    if (DateTime.Now >= _preparingCd)
                    {
                        ...
                        InformDefender();
                        SyncTime();
                        foreach (VnXPlayer c in Enumerable.ToList<VnXPlayer>(Initialize.ReallifePlayers))
                        {
                            _RootCore_.VenoX.TriggerClientEvent(c, "gw:aa", _gangwarArea.Position.X, _gangwarArea.Position.Y, _gangwarArea.Position.Z, _gangwarArea.Radius, _gangwarArea.Rotation, GangwarManager.AttBlipRgba);
                        }
                    }
                }
                        ...
                        foreach (PlayerEntry entry in PlayerList)
                        {
                            if (entry.GetFaction() == AttackerId)
                            {
                                if (_gangwarArea.Tk.Distance(entry.Player.Position) <= GangwarManager.TkRange)
                                {
                                    ...
                                    {
                                        TkCounter = 0;
                                        Faction.CreateCustomBadFactionMessage(RageApi.GetHexColorcode(0, 255, 0) + entry.Player.CharacterUsername + " hat den TK beesetzt!", AttackerId);
                                    }
                                    break;
                                }
                            }
        ...
        public void AddPlayer(VnXPlayer player)
        {
            try
            {
                ...
                _RootCore_.VenoX.TriggerClientEvent(player, "gw:showUp", true, Faction.GetFactionNameById(AttackerId), Faction.GetFactionNameById(DefenderId), attackRgb.R, attackRgb.G, attackRgb.B, defenderRgb.R, defenderRgb.G, defenderRgb.B);
        ...
        public void InformAllThatPlayerJoined()
        {
            try
            {
                foreach (VnXPlayer c in Enumerable.ToList<VnXPlayer>(Initialize.ReallifePlayers))
                {
                    _RootCore_.VenoX.TriggerClientEvent(c, "gw:joinedPlayer", GetFactionInfo(AttackerId), GetFactionInfo(DefenderId));
                }
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
            ...
            try
            {
                switch (CurrentState)
                {
                    case RoundStates.Preparing:
                    {
                        ...
                        _RootCore_.VenoX.TriggerClientEvent(player, "gw:updateTime", (int)leftTime);
                        break;
                    }
        ...
        public void SyncStats(VnXPlayer player, PlayerEntry playerEntry)
        {
            try
            {
                ...
                {
                    _RootCore_.VenoX.TriggerClientEvent(player, "gw:updateStats", playerEntry.TotalDamage, playerEntry.TotalKills, GetFactionInfo(AttackerId), GetFactionInfo(DefenderId));
                }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
        ...
        public void ResetPlayer(PlayerEntry playerEntry)
        {
            try
            {
                ...
                if (!playerEntry.IsLeft)
                {
                    _RootCore_.VenoX.TriggerClientEvent(playerEntry.Player, "gw:showUp", false, "FAC 1", "FAC 2", 255, 255, 255, 255, 255, 255);
        ...
        public void KillPlayer(PlayerEntry entry)
        {
            try
            {
                ...
                foreach (PlayerEntry pentrys in PlayerList)
                {
