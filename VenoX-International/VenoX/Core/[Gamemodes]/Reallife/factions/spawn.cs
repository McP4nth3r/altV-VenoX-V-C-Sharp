...
namespace VenoX.Core._Gamemodes_.Reallife.factions
{
    ...
    {
        //ColShapeModel SpawnLSPD = RageAPI.CreateColShapeSphere(new Vector3(469.8354f, -985.0742f, 33.89248f), 3, Core.Globals.Main.REALLIFE_DIMENSION);
        //ColShapeModel SpawnARMY = RageAPI.CreateColShapeSphere(new Vector3(468.65933f, -3205.8594f, 9.784668f), 3, Core.Globals.Main.REALLIFE_DIMENSION);
        //ColShapeModel SpawnARMY2 = RageAPI.CreateColShapeSphere(new Vector3(-2089.4636f, 3273.7056f, 32.801514f), 3, Core.Globals.Main.REALLIFE_DIMENSION);
        ...
        public static void SpawnPlayerOnSpawnpoint(VnXPlayer player)
        {
            try
            {
                ...
                _RootCore_.VenoX.TriggerClientEvent(player, "start_screen_fx", "RaceTurbo", 2000, false);
                player.Dimension = _Globals_.Initialize.ReallifeDimension;
                player.Freeze = true;
                ...
                switch (player.Reallife.SpawnLocation)
                {
                    ...
                    case "Basis-2":
                        ...
                        break;
                    case "House":
                        foreach (HouseModel house in House.HouseList.ToList().Where(house => player.Reallife.HouseRent > 0 || player.CharacterUsername == house.Owner))
                        {
