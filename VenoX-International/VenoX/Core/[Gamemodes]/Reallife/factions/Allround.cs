...
using VenoX.Core._Gamemodes_.Reallife.register_login;
...
namespace VenoX.Core._Gamemodes_.Reallife.factions
{
    public class Allround : IScript
    {
        ...
        private static readonly Dictionary<string, WeaponModel> _fgunsWeapons = new Dictionary<string, WeaponModel>
        {
        ...
        public static void UpdateFgunsWindow(VnXPlayer player)
        {
            if (IsStateFaction(player))
            {
                ...
                _RootCore_.VenoX.TriggerClientEvent(player, "fguns:ForceStateWindowUpdate",
        ...
        public static void FgunsSelectWeapon(VnXPlayer player, string weapon)
        {
            try
            {
                ...
                {
                    ConsoleHandling.OutputDebugString("fguns:selectweapon not found weapon ID : " + weapon);
                    return;
                }
        ...
        public static bool OnPlayerEnterColShapeModel(ColShapeModel shape, VnXPlayer player)
        {
            try
            {
                if (shape == LspdcolFguns || shape == FbicolFguns)
                {
                    ...
                    {
                        WaffenlagerModel fweapon = Fraktionswaffenlager.GetWaffenlagerById(Constants.FactionLspd);
                        _RootCore_.VenoX.TriggerClientEvent(player, "fguns:Open", true);
            ...
            try
            {
                ...
                if (playerFraktion == Constants.FactionLcn || IsStateFaction(player) || Admin.HaveAdminRights(player))
                {
                    ...
                    if (LcnTeleportBaseEnter.Distance(player.Position) < 1.25f)
                    {
                        ...
                        {
                            Notification.DrawTranslatedNotification(player, Notification.Types.Warning, "Bitte warte 3 Sekunden!");
                            return true;
                        }
                ...
                if (playerFraktion == Constants.FactionSamcro || IsStateFaction(player) || Admin.HaveAdminRights(player))
                {
                    ...
                    /*if (SAM_Teleport_Base_Enter.Distance(player.Position) < 1.25f)
                    {
                        if(player.Reallife.LastFactionTeleport > DateTime.Now)
                        {
                            _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Warning, "Bitte warte 3 Sekunden!");
                            return true;
                        }
                    ...
                    else if (SAM_Teleport_Base_Exit.Distance(player.Position) < 1.25f && player.Dimension == Constants.FACTION_SAMCRO)
                    {
                        ...
                        player.Dimension = Core.Globals.Main.REALLIFE_DIMENSION+ player.Language;
                        return true;
                    }*/
                }
            }
        ...
        public static void Selfuninvite(VnXPlayer player)
        {
            ...
            {
                if (player.Reallife.Faction == Constants.FactionNone) return;
                Faction.CreateFactionInformation(player.Reallife.Faction, player.CharacterUsername + " hat die Fraktion verlassen...");
                player.Reallife.Faction = Constants.FactionNone;
                player.Reallife.SpawnLocation = "noobspawn";
                Notification.DrawNotification(player, Notification.Types.Info, "Du hast dich selbst uninvitet!");
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
            ...
            try
            {
                ...
                if (player.Reallife.Faction != Constants.FactionNone && player.Reallife.FactionRank >= 4)
                {
                    if (target.Reallife.Faction == Constants.FactionNone)
                    {
                            ...
                            player.SendTranslatedChatMessage(RageApi.GetHexColorcode(0, 150, 0) + "Du hast den Spieler " + target.CharacterUsername + " in deine Fraktion aufgenommen!");
                        
                    }
                    else
                    {
                        Notification.DrawNotification(player, Notification.Types.Error, "Der Spieler ist bereits in einer Fraktion!");
                    }
                }
                else
                {
                    Notification.DrawNotification(player, Notification.Types.Error, "Erst ab Rank 4 möglich!");
                }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
        ...
        public static void UninviteFromFactionPlayer(VnXPlayer player, string targetName)
        {
            try
            {
                ...
                if (player.Reallife.Faction != Constants.FactionNone && player.Reallife.FactionRank >= 4)
                {
                    ...
                    {
                        if (target.CharacterUsername == player.CharacterUsername)
                        {
                            Notification.DrawNotification(player, Notification.Types.Error, "Du kannst dich nicht selbst Rauswerfen... Nutze /selfuninvite");
                            return;
                        }
                        if (target.Reallife.FactionRank < 5)
                        {
                            ...
                            player.SendTranslatedChatMessage(RageApi.GetHexColorcode(0, 175, 0) + "Du hast den Spieler " + target.CharacterUsername + " aus deiner Fraktion entfernt!");
                        }
                        else
                        {
                            Notification.DrawNotification(player, Notification.Types.Error, "Du kannst keinen Leader un-inviten!");
                        }
                    }
                    else
                    {
                        Notification.DrawNotification(player, Notification.Types.Error, "Der Spieler ist nicht in deiner Fraktion!");
                    }
                }
                else
                {
        ...
        public static void SetPlayerFraktionsRang(VnXPlayer player, string targetName, int number)
        {
            try
            {
                ...
                if (player.Reallife.FactionRank >= 4 && player.Reallife.Faction > Constants.FactionNone)
                {
                    ...
                    if (target.Reallife.FactionRank == 5)
                    {
                        Notification.DrawNotification(player, Notification.Types.Error, "Du kannst einen Leader/Co-Leader nicht seinen Rang ändern!");
                        return;
                    }
                    if (target.Reallife.FactionRank == number)
                    {
                        Notification.DrawNotification(player, Notification.Types.Error, "Der Spieler hat bereits diesen Rang...");
                        return;
                    }
                    ...
                    if (target.Reallife.FactionRank < number)
                    {
                        target.SendTranslatedChatMessage(RageApi.GetHexColorcode(0, 175, 0) + "Glückwunsch, du wurdest soeben von " + player.CharacterUsername + " zum " + rankString + " befördert!");
                    }
                    else
                    {
                        target.SendTranslatedChatMessage(RageApi.GetHexColorcode(175, 0, 0) + "Du wurdest soeben von " + player.CharacterUsername + " zum " + rankString + " degradiert!");
                    }
                    player.SendTranslatedChatMessage(RageApi.GetHexColorcode(0, 175, 0) + "Du hast " + target.CharacterUsername + " soeben Rang " + rankString + " ( " + number + " ) gegeben!");
                    target.Reallife.FactionRank = number;

                }
                else
                {
                    Notification.DrawNotification(player, Notification.Types.Error, "Du bist nicht befugt!");
                }
            }
            catch
            {
        ...
        public void GoDutybadServer(VnXPlayer player, string state)
        {
            try
            {
                ...
                if (player.IsDead) { Notification.DrawNotification(player, Notification.Types.Error, "Diese Aktion ist derzeit nicht Möglich!"); return; }
        ...
        public void GoDutyIPlayer(VnXPlayer player)
        {
            try
            {
                ...
                if (player.IsDead)
                {
                    Notification.DrawNotification(player, Notification.Types.Error, "Diese Aktion ist derzeit nicht Möglich!");
                    return;
                }
                if (IsStateFaction(player))
                {
