    ...
    public class PaynSpray
    {
        ...
        public static bool OnPlayerEnterColShapeModel(IColShape forpaynspray, VnXPlayer player)
        {
            try
            {
                ...
                if (vehicle.Npc) { Notification.DrawNotification(player, Notification.Types.Info, "Du kannst kein NPC-Fahrzeug Reparieren!"); return true; }
                int playerMoney = player.Reallife.Money;
                if (playerMoney >= 180)
                {
                    player.Reallife.Money -= 180;
                    vehicle.Repair();
                    Notification.DrawNotification(player, Notification.Types.Info, "Fahrzeug Repariert! [180 $]");
                }
                else Notification.DrawNotification(player, Notification.Types.Error, "Du hast nicht genug Geld!");
                return true;
            }
