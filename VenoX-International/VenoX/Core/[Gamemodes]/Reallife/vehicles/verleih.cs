...
namespace VenoX.Core._Gamemodes_.Reallife.vehicles
{
    public class Verleih : IScript
    {
        ...
        public void Update_VenoX_Rentals(VnXPlayer player)
        {
            try
            {
                ...
                if (player.VnxGetElementData<int>(CurrentVehicleRent) == 0)
                {
                    ...
                    {
                        player.VnxSetStreamSharedElementData(EntityData.PlayerMoney, playerMoney - FaggioCosts);
                        _RootCore_.VenoX.TriggerClientEvent(player, "VnX_UpdateRent", player);
                    }
                    else
                    {
        ...
        public void destroy_Fahrzeug_verleih(VnXPlayer player)
        {
            try
            {
                foreach (_RootCore_.Models.VehicleModel vehicle in Enumerable.ToList<_RootCore_.Models.VehicleModel>(_Globals_.Initialize.ReallifeVehicles))
                {
                    if (vehicle.Rented && vehicle.Owner == player.CharacterUsername)
                    {
        ...
        public void stop_Fahrzeug_Verleih(VnXPlayer player)
        {
            try
            {
                ...
                {
                    Notification.DrawNotification(player, Notification.Types.Error, "VenoX Rental : <br>Du hast bei uns kein Fahrzeug gemietet!");
                }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
