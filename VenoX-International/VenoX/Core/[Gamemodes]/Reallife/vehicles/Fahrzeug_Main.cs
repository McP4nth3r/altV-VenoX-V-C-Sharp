    ...
    public class FahrzeugMain : IScript
    {
        ...
        public void ShowIVehicleMenu(VnXPlayer player)
        {
            try
            {
                if (player.IsInVehicle)
                {
                    ...
                    _RootCore_.VenoX.TriggerClientEvent(player, "showIVehicleMenu");
        ...
        public void Vehiclelist(VnXPlayer player)
        {
            try
            {
                ...
                {
                    _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Nur alle 30 Sekunden möglich!");
                    return;
                }
                ...
                {
                    if (veh != null && veh.Owner == player.CharacterUsername)
                    {
        ...
        public void ParkLocalIVehicle(VnXPlayer player)
        {
            try
            {
                if (player.IsInVehicle)
                {
                    ...
                    if (localVehicle.Owner == player.CharacterUsername)
                    {
                ...
                if (player.IsInVehicle)
                {
                    ...
                    if (localVehicle.Owner ==player.Username)
                    {
                        if (player.Reallife.Money >= 200)
                        {
                            ...
                            localIVehicle.position = localVehicle.vnxGetElementData<Position>(Core.Globals.EntityData.VEHICLE_OWNER);
                            localIVehicle.Rotation = localVehicle.SpawnRot;
                            player.vnxSetElementData(Core.Globals.EntityData.PLAYER_MONEY, player.Reallife.Money - 200);
            ...
            try
            {
                ...
                if (Vehicle != null)
                {
                    if (Vehicle.Owner ==player.Username)
                    {
                        ...
                        {
                            IVehicle.Repair();
                            IVehicle.position = Vehicle.vnxGetElementData<Position>(Core.Globals.EntityData.VEHICLE_OWNER);
                            IVehicle.Rotation = Vehicle.SpawnRot;
                            Vehicle.Dimension = (uint)Vehicle.vnxGetElementData<int>(Core.Globals.EntityData.VEHICLE_DIMENSION);
