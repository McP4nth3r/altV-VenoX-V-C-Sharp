    ...
    public class Airport : IScript
    {
        ...
        public static void OnPlayerExitVehicle(VehicleModel vehClass, VnXPlayer player)
        {
            try
            {
                ...
                player.Reallife.JobStage = 0;
                player.Dimension = _Globals_.Initialize.ReallifeDimension + player.Language;
                ConsoleHandling.OutputDebugString("JobStage 2: " + player.Reallife.JobStage);
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
            ...
            try
            {
                switch (stage)
                {
                    case 1:
                    {
                        ...
                        _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Fly to the drop-off point!");
                        ...
                        airportjobPlane.Owner = player.CharacterUsername;
                    ...
                    case 2 when player.Reallife.AirportJobLevel <= 50:
                        _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Error, "You need at least job level 50!"); return;
                    case 2:
                    {
                        int randomjobdim = VnX.GetRandomNumber(1, 99999);
                        _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Fly to the drop-off point!");
