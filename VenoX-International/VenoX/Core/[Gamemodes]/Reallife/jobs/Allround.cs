...
namespace VenoX.Core._Gamemodes_.Reallife.jobs
{
    public class Allround : IScript
    {
                ...
                if (shape == CityTransportCol)
                {
                    switch (player.Reallife.Job)
                    {
                        case Constants.JobCityTransport:
                            ...
                            break;
                        case "Arbeitslos":
                            VnX.DrawWindow(player, "Venox City Transport", "Hallo " + player.CharacterUsername + ",<br>willkommen bei Venox City Transport!<br>Du liebst es zu Fahren ?<br>Du liebst es mit anderen Menschen in Kontakt zu kommen ?<br>Dann bist du hier genau Richtig!<br>Möchtest du deine Karriere als Transporter Starten ?", "Job Annehmen", "Job Ablehnen");
                            break;
                        default:
                            _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Du hast bereits einen Job! Nutze /quitjob um deinen Job zu beenden!");
                            break;
                    }
                }
                else if (shape == AirportJobCol)
                {
                    switch (player.Reallife.Job)
                    {
                        ...
                        case Constants.JobNone:
                            VnX.DrawWindow(player, "LS Airport", "Hallo " + player.CharacterUsername + ",<br>willkommen bei Venox City Airport!<br>Du liebst es zu Fliegen?<br>Du liebst es mit anderen Menschen in Kontakt zu kommen ?<br>Dann bist du hier genau Richtig!<br>Möchtest du deine Karriere als Pilot Starten ?", "Job Annehmen", "Job Ablehnen");
                            break;
                        default:
                            _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Du hast bereits einen Job! Nutze /quitjob um deinen Job zu beenden!");
                            break;
                ...
                else if (shape == BusJobCol)
                {
                    switch (player.Reallife.Job)
                    {
                        case Constants.JobBus:
                            ...
                            break;
                        case Constants.JobNone:
                            VnX.DrawWindow(player, "VenoX Busdepot", "Hallo " + player.CharacterUsername + ",<br>willkommen beim VenoX City Busdepot!<br>Du liebst es mit einem 500.000$ Benz zu fahren?<br>Du liebst es jedentag heiße Frauen in deinem Fahrzeug zu haben?<br>Dann werde heute noch Busfahrer!", "Job Annehmen", "Job Ablehnen");
                            break;
                        default:
                            _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Du hast bereits einen Job! Nutze /quitjob um deinen Job zu beenden!");
                            break;
        ...
        public void trigger_job_window_1_buttons(VnXPlayer player, int stage)
        {
            try
            {
                switch (player.Reallife.Job)
                {
                    ...
                    default:
                        _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Error, "du bist bei keinem Job");
                        _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Error, "dein Job " + player.Reallife.Job);
                        break;
                }
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        ...
        public static void QuitJobServer(VnXPlayer player)
        {
            try
            {
                if (player.Reallife.Job != Constants.JobNone)
                {
                    ...
                    _Globals_.Notification.DrawNotification(player, _Globals_.Notification.Types.Info, "Du bist nun Arbeitslos.");
                }
            }
            catch(Exception ex){ExceptionHandling.CatchExceptions(ex);}
        }
        ...
        public static VehicleModel CreateJobVehicle(VnXPlayer player, AltV.Net.Enums.VehicleModel veh, Vector3 position, Vector3 rotation, string job, int dimension = _Globals_.Initialize.ReallifeDimension, bool warpIntoVehicle = true)
        {
            try
            {
                ...
                jobVehicle.Owner = player.CharacterUsername;
                jobVehicle.Kms = 0;
                jobVehicle.Gas = 100;
                jobVehicle.Job = job;
        ...
        public static void OnPlayerDisconnect(VnXPlayer player)
        {
            try
            {
                ...
                foreach (VehicleModel vehClass in Initialize.ReallifeVehicles.ToList<VehicleModel>())
                {
