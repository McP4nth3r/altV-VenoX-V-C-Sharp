...
using VenoX.Core._Gamemodes_.Shooter.Models;
...
namespace VenoX.Core._Gamemodes_.Shooter.Lobby
{
    public class Lobby
    {
        // Settings
        ...
        public static void PutPlayerInRound(VnXPlayer player)
        {
            ...
            player.Dimension = Initialize.ShooterDimension;
            ...
            vehicle.Dimension = Initialize.ShooterDimension;
        }
        public static void OnPlayerDeath(VnXPlayer player)
        {
            TakePlayerFromRound(player);
            int counter = 0;
            foreach (VnXPlayer players in Enumerable.ToList<VnXPlayer>(Initialize.ShooterPlayers))
            {
            ...
            if (counter <= 1)
            {
                ...
                VnXPlayer winner = Enumerable.ToList<VnXPlayer>(Initialize.ShooterPlayers).FirstOrDefault(x => x.Shooter.IsAlive);
                if (winner is not null) winnerName = player.CharacterUsername;
        ...
        private static void ShowWinner(string winnerName)
        {
            try
            {
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(Initialize.ShooterPlayers))
                {
                    _RootCore_.VenoX.TriggerClientEvent(player, "Shooter:ShowWinner", winnerName);
                }
            }
