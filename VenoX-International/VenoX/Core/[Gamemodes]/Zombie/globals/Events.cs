...
namespace VenoX.Core._Gamemodes_.Zombie.globals
{
    public class Events : IScript
    {
        ...
        private static readonly List<AltV.Net.Enums.VehicleModel> _randomVehicleList = new List<AltV.Net.Enums.VehicleModel>
        {
        ...
        public static void CreateVehicleSpawnChance(VnXPlayer player, Vector3 position, Vector3 rotation)
        {
            try
            {
                ...
                ConsoleHandling.OutputDebugString("Chance  : " + randomnumb);
                if (randomnumb == 10)
                {
                    ConsoleHandling.OutputDebugString("Position  : " + position);
                    ConsoleHandling.OutputDebugString("Rotation  : " + rotation);
                    ...
                    veh.Dimension = _Globals_.Initialize.ZombiesDimension;
                    
                    veh.NotSave = true;
                    veh.EngineOn = true;
                    veh.Owner = player.CharacterUsername;
                    veh.Kms = 0;
                    veh.Gas = 1000f;
        ...
        public static void OnPlayerDisconnect(VnXPlayer player)
        {
            try
            {
                ...
                if (_Globals_.Initialize.ZombiePlayers.Count <= 0)
                {
