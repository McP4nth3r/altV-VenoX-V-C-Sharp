...
namespace VenoX.Core._Gamemodes_.Zombie.World
{
    public class Main : IScript
    {
        //public static Position PLAYER_SPAWN_NOOBSPAWN = new Position(-2132.323f, 2821.959f, 34.84159f); // Noobspawn
        public static Position PlayerSpawnNoobspawn = new Position(0, 0, 72); // Noobspawn
        public static int TimeIntervalZombies = 10; // Zeit in sekunden wie oft Zombies spawnen sollten.
        public static int ZombieAmmountEachSpawn = 2; // Zombies die Pro Spawn-Function Aufruf spawnen sollen.
        ...
        public static void OnSelectedZombieGM(VnXPlayer player)
        {
            try
            {
                ...
                int randomnumb = random.Next(0, PlayerSpawns.Count);
                player.SpawnPlayer(PlayerSpawns[randomnumb]);
                player.Dimension = _Globals_.Initialize.ZombiesDimension;
                ...
                else
                {
                    ...
                    foreach (VnXPlayer nearbyPlayers in player.NearbyPlayers.ToList())
                    {
                        ...
                        else
                        {
                            ...
                            _RootCore_.VenoX.TriggerClientEvent(player, "Zombies:Sync", false);
                            _RootCore_.VenoX.TriggerClientEvent(nearbyPlayers, "Zombies:Sync", false);
                        }
                    }
                }
            }
            ...
            try
            {
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.ZombiePlayers))
                {
                    if (player != null)
                    {
                        ...
                        if (player.Zombies.IsSyncer) _RootCore_.VenoX.TriggerClientEvent(player, "Zombies:Sync", true, player.NearbyPlayers.Count);
                    ...
                    foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.ZombiePlayers))
                    {
                        if (player != null && player.Exists)
                        {
                            if (player.Position.Distance(zombieClass.Position) < 150 && !zombieClass.IsDead)
                            {
                                ...
                                _RootCore_.VenoX.TriggerClientEvent(player, "Zombies:MoveToTarget", zombieClass.Id, zombieClass.SkinName, zombieClass.Position, zombieClass.Rotation, zombieClass.TargetEntity, move);
                            }
                            else
                            {
                                if (player.Zombies.NearbyZombies.Contains(zombieClass))
                                {
                                    ...
                                    _RootCore_.VenoX.TriggerClientEvent(player, "Zombies:DeleteTempZombieById", zombieClass.Id);
                                }
                            }
                        }
                    }
        ...
        public static void OnUpdate()
        {
            try
            {
                ...
                if (TimeToGetNewTarget <= DateTime.Now)
                {
