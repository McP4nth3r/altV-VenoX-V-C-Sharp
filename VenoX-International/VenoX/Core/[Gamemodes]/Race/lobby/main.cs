...
using VenoX.Core._Gamemodes_.Race.globals;
using VenoX.Core._Gamemodes_.Race.model;
...
namespace VenoX.Core._Gamemodes_.Race.lobby
{
    public class Main : IScript
    {
        ...
        private static void DeleteEverything()
        {
            try
            {
                ...
                foreach (VnXPlayer racePlayers in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.RacePlayers))
                {
        ...
        private static void InitializePlayerDatas()
        {
            try
            {
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.RacePlayers))
                {
                    if (!player.Race.IsRacing)
                    {
                        ...
                        player.Dimension = _Globals_.Initialize.RaceDimension;
                        double leftTime = (DateTime.Now - DateTime.Now.AddMinutes(RaceRoundMinutes)).TotalSeconds * -1;
                        _RootCore_.VenoX.TriggerClientEvent(player, "Race:StartTimer", leftTime, 3);
                        VehicleModel vehicle = (VehicleModel)Alt.CreateVehicle(CurrentMap.PlayerVehicleHash, spawnpoint, rotation);
                        _RootCore_.VenoX.TriggerClientEvent(player, "Vehicle:DisableEngineToggle", false); // Disable Auto-TurnOn for Vehicle.
        ...
        public static void SyncPlayerPlaceInRound()
        {
            try
            {
                ...
                foreach (VnXPlayer player in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.RacePlayers))
                {
                    _RootCore_.VenoX.TriggerClientEvent(player, "Race:ClearPlayerList", player.CharacterUsername);
                    foreach (VnXPlayer players in Enumerable.OrderBy<VnXPlayer, int>(_Globals_.Initialize.RacePlayers, p => p.Race.CurrentMarker).Reverse())
                    {
                        counter++;
                        players.Race.RoundPlace = counter;
                        _RootCore_.VenoX.TriggerClientEvent(player, "Race:FillPlayerList", players.CharacterUsername);
                    }
                }
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        ...
        public static async void OnClientRaceFinish(VnXPlayer player)
        {
            try
            {
                if (RaceFirstWinner == String.Empty) RaceFirstWinner = player.CharacterUsername;
                else if (RaceSecondWinner == String.Empty) RaceSecondWinner = player.CharacterUsername;
                else if (RaceThirdWinner == String.Empty) RaceThirdWinner = player.CharacterUsername;

                player.Vehicle.EngineOn = false;
                player.Freeze = true;
                player.Race.IsRacing = false;

                string translatedText = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)player.Language, "hat das Rennen beendet!");
                foreach (VnXPlayer players in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.RacePlayers))
                {
                    players.SendChatMessage(RageApi.GetHexColorcode(0, 200, 255) + player.CharacterUsername + RageApi.GetHexColorcode(255, 255, 255) + " " + translatedText);
                }
                RacePlayersFinished.Add(player);
                if (RacePlayersFinished.Count == _Globals_.Initialize.RacePlayers.Count)
                {
                    string translatedText1 = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)player.Language, " [Race] : Gewinner - 1 :  ");
                    string translatedText2 = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)player.Language, " [Race] : Gewinner - 2 :  ");
                    string translatedText3 = await global::VenoX.Core._Language_.Main.GetTranslatedTextAsync((global::VenoX.Core._Language_.Constants.Languages)player.Language, " [Race] : Gewinner - 3 :  ");
                    foreach (VnXPlayer players in Enumerable.ToList<VnXPlayer>(_Globals_.Initialize.RacePlayers))
                    {
        ...
        private static void PutPlayerInRound(VnXPlayer player)
        {
            try
            {
                if (!CanPlayerJoin() || _Globals_.Initialize.RacePlayers.Count >= 2)
                {
        ...
        public static void OnSelectedRaceGM(VnXPlayer player)
        {
            try
            {
                ...
                Functions.SendRaceRoundMessage(RageApi.GetHexColorcode(0, 200, 0) + player.CharacterUsername + " hat die Race runde betreten.");
            }
            catch (Exception ex) { ExceptionHandling.CatchExceptions(ex); }
        }
