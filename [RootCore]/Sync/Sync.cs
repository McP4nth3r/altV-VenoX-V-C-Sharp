...
namespace VenoXV._RootCore_.Sync
{
    public class Sync
    {
                    ...
                    if (playerClass.Position.Distance(new Vector3(labels.PosX, labels.PosY, labels.PosZ)) <= RenderDistance && (labels.Dimension == playerClass.Dimension || labels.Dimension == Dimension.GlobalDimension))
                    {
                        if (labels.VisibleOnlyFor == null || labels.VisibleOnlyFor == playerClass)
                        {
                            if (!playerClass.Sync.CurrentLabels.Contains(labels))
                            {
                                ...
                                // House Sync.
                                else if (labels.IsHouseLabel) VenoX.TriggerClientEvent(playerClass, "Sync:LoadTextLabels", labels.Id, await House.GetHouseLabelText(null, (Main.Languages)playerClass.Language, labels.HouseLabelId), labels.PosX, labels.PosY, labels.PosZ, labels.Font, labels.ColorR, labels.ColorG, labels.ColorB, labels.ColorA, labels.Dimension, labels.Range);
                                // Normal Translated Sync.
                                else VenoX.TriggerClientEvent(playerClass, "Sync:LoadTextLabels", labels.Id, await Main.GetTranslatedTextAsync((Main.Languages)playerClass.Language, labels.Text), labels.PosX, labels.PosY, labels.PosZ, labels.Font, labels.ColorR, labels.ColorG, labels.ColorB, labels.ColorA, labels.Dimension, labels.Range);
        ...
        public static void DeleteVehicleThreadSafe()
        {
            ...
            {
                //int i = 0;
                foreach (VehicleModel vehClass in _Globals_.Main.AllVehicles.ToList())
                {
                    if (_Globals_.Main.AllVehicles.ToList().Contains(vehClass) && vehClass.MarkedForDelete)
                    {
                        //Debug.OutputDebugString("DeleteVehicleThreadSafe : " + i++);
                        _Globals_.Main.AllVehicles.Remove(vehClass);
                        vehClass.Remove();
                    }
                }
            }
