    ...
    public class House : IScript
    {
        ...
        public void LoadDatabaseHouses()
        {
            ...
            foreach (HouseModel houseModel in houseList)
            {
                ...
                //houseModel.houseLabel = //ToDo: ClientSide erstellen NAPI.TextLabel.CreateTextLabel(houseLabelText, houseModel.position, 20.0f, 0.75f, 4, new Rgba(255, 255, 255), false, houseModel.dimension);
        ...
        public static bool HasPlayerHouseKeys(IPlayer player, HouseModel house)
        {
            try
            {
                return (player.Name == house.owner || player.vnxGetElementData<int>(EntityData.PLAYER_RENT_HOUSE) == house.id);
            }
            catch { return false; }
        }
            ...
            try
            {
                if (house.status == Constants.HOUSE_STATE_BUYABLE)
                {
                    if (player.vnxGetElementData<int>(EntityData.PLAYER_BANK) >= house.price)
                    {
                        ...
                        logfile.WriteLogs("house",player.Name + " hat sich Haus ID " + house.id + " gekauft für " + house.price + " $ ");
