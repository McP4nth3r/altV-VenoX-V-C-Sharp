    ...
    public class Motorrad_Führerschein : IScript
    {
        ...
        public static void Start_Motorrad_Führerschein(IPlayer player)
        {
            try
            {
                ...
                Core.VnX.IVehiclevnxSetSharedData(PruefungsAuto, "kms", 0);
                Core.VnX.IVehiclevnxSetSharedData(PruefungsAuto, "gas", 100);
               ...
               dxLibary.VnX.DrawZielBlip(player, "Checkpoint [ Führerschein ]", Pruefungs_Marker_Motorrad[1], 611, 3, player.Dimension);
                ...
                Motorrad_Führerschein_Abgabe_Marker.SetData("Name",player.Name);
            }
            catch
            {
            }
        ...
        public static void TriggerToNextPruefungsMarker(IPlayer player, int counter)
        {
            try
            {
                if (counter == 31)
                {
                    ...
                    if (Motorrad_Führerschein_Abgabe_Marker.vnxGetElementData<string>("Name") ==player.Name)
                    {
        ...
        public static void OnPlayerEnterIColShape(IColShape shape, IPlayer player)
        {
            try
            {
                ...
                if (player.IsInVehicle)
                {
                    ...
                    if (Vehicle.vnxGetElementData<bool>("PRUEFUNGS_AUTO") == true && player.vnxGetElementData<int>("Marker_Pruefung") != 31 && Vehicle.vnxGetElementData<string>("PRUEFUNGS_AUTO_BESITZER") ==player.Name && player.vnxGetElementData<string>("PRUEFUNGS_NAME") == "BIKE")
                    {
        ...
        public void OnPlayerExitIVehicle(IVehicle Vehicle, IPlayer player, byte seat)
        {
            try
            {
                if (Vehicle.vnxGetElementData<bool>("PRUEFUNGS_AUTO") == true && Vehicle.vnxGetElementData<string>("PRUEFUNGS_AUTO_BESITZER") ==player.Name && player.vnxGetElementData<int>("Marker_Pruefung") >= 0 && player.vnxGetSharedData<bool>("PLAYER_DRIVINGSCHOOL") == true && player.vnxGetElementData<string>("PRUEFUNGS_NAME") == "BIKE")
                {
                    player.SetData("Marker_Pruefung", 0);
                    dxLibary.VnX.DestroyRadarElement(player, "Blip");
                    ...
                    Vehicle.Remove();
                    if (Motorrad_Führerschein_Abgabe_Marker != null && Motorrad_Führerschein_Abgabe_Marker.vnxGetElementData<string>("Name") ==player.Name)
                    {
                        AltV.Net.Alt.RemoveColShape(Motorrad_Führerschein_Abgabe_Marker);
                    }
                    return;
