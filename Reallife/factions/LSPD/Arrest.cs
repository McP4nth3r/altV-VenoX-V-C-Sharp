                ...
                if (kaution > 0)
                {
                    if (player.vnxGetElementData<int>(EntityData.PLAYER_KNASTZEIT) > 0)
                    {
                        if (player.vnxGetElementData<int>(EntityData.PLAYER_MONEY) >= kaution)
                        {
                            ...
                            player.vnxSetSharedElementData<object>(EntityData.PLAYER_MONEY, player.vnxGetElementData<int>(EntityData.PLAYER_MONEY) - kaution);
                            player.vnxSetSharedElementData<object>(EntityData.PLAYER_KNASTZEIT, 0);
                        ...
                        else if (player.vnxGetElementData<int>(EntityData.PLAYER_BANK) >= kaution)
                        {
                            ...
                            Database.SetFactionStats(Constants.FACTION_POLICE, fkasse.money + kaution, fkasse.weed, fkasse.koks, fkasse.mats);
                            Faction.CreateCustomStateFactionMessage(RageAPI.GetHexColorcode(0, 200, 0) + player.GetVnXName<string>() + " hat die Kaution in Höhe von " + kaution + "$ bezahlt!");
                            player.vnxSetSharedElementData<object>(EntityData.PLAYER_BANK, player.vnxGetElementData<int>(EntityData.PLAYER_BANK) - kaution);
                            AntiCheat_Allround.SetTimeOutTeleport(player, 7000);
                            player.vnxSetSharedElementData<object>(EntityData.PLAYER_KNASTZEIT, 0);
                            player.Position = new Position(427.5651f, -981.0995f, 30.71008f);
                            player.Dimension = 0;
        ...
        public static void TiePlayerinIVehicle(IPlayer player, string target_name)
        {
            try
            {
                if (Allround.isStateFaction(player))
                {
                    ...
                    {
                        target.vnxSetElementData<object>(EntityData.PLAYER_HANDCUFFED, true);
                    ...
                    {
                        target.vnxSetElementData<object>(EntityData.PLAYER_HANDCUFFED, false);
                    ...
                    else
                    {
                        if (target.vnxGetElementData<bool>(EntityData.PLAYER_HANDCUFFED) == false)
                        {
                            ...
                            player.vnxSetElementData<object>(EntityData.PLAYER_ANIMATION, true);
                            target.vnxSetElementData<object>(EntityData.PLAYER_HANDCUFFED, true);
                            dxLibary.VnX.DrawNotification(player, "info", "Du hast " + target.GetVnXName<string>() + " gefesselt!");
                            dxLibary.VnX.DrawNotification(target, "warning", player.GetVnXName<string>() + " hat dich gefesselt!");
                        ...
                        else
                        {
                            ...
                            */
                            ...
                            target.vnxSetElementData<object>(EntityData.PLAYER_HANDCUFFED, false);
                            target.vnxSetElementData<object>(EntityData.PLAYER_ANIMATION, false);
                            ...
                            if (target.Position.Distance(arrestpositioncar) < 3.5f)
                            {
                                ...
                                if (Allround.isStateIVehicle(Vehicle))
                                {
                                    ...
                                    if (kaution == "1")
                                    {
                                        ...
                                        target.vnxSetElementData<object>(EntityData.PLAYER_KNASTZEIT, target.vnxGetElementData<int>(EntityData.PLAYER_WANTEDS) * 4);
                                        target.vnxSetElementData<object>(EntityData.PLAYER_KAUTION, KostenProWanted * target.vnxGetElementData<int>(EntityData.PLAYER_WANTEDS));
                                    ...
                                    else
                                    {
                                        ...
                                        target.vnxSetElementData<object>(EntityData.PLAYER_KNASTZEIT, target.vnxGetElementData<int>(EntityData.PLAYER_WANTEDS) * 5);
                                        target.vnxSetElementData<object>(EntityData.PLAYER_KAUTION, 0);
                                        anzeigen.Usefull.VnX.UpdateHUD(target);
                                        target.vnxSetSharedElementData<object>(EntityData.PLAYER_WANTEDS, 0);
                                        anzeigen.Usefull.VnX.RemoveAllWeapons(target);
                                        Random random = new Random();
        ...
        public void Stellen_server_event(IPlayer player)
        {
            try
            {
                ...
                else
                {
                    ...
                    player.vnxSetElementData<object>(EntityData.PLAYER_KNASTZEIT, player.vnxGetElementData<int>(EntityData.PLAYER_WANTEDS) * 4);
                    player.vnxSetElementData<object>(EntityData.PLAYER_KAUTION, player.vnxGetElementData<int>(EntityData.PLAYER_WANTEDS) * KostenProWanted);
