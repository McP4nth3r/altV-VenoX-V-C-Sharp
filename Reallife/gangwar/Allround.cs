                                    ...
                                    if (player.vnxGetElementData<int>(EntityData.PLAYER_RANK) >= GangwarManager.MIN_RANK_ATTACK)
                                    {
                                        ...
                                        if (_gangwarManager.GetFactionCount(area.IDOwner) >= GangwarManager.MIN_COUNT_PLAYER)
                                        {
                                            ...
                                            if (area.isAttackable())
                                            {
                                                ...
                                                Reallife.Core.RageAPI.SendChatMessageToAll("!{200,0,0}Ein Gangwar wird vorbereitet!");
                                                factions.Faction.CreateCustomFactionInformation(_gangwarManager.currentArea.GetCurrentRound().AttackerId, "!{0,200,0}" +player.Name + " hat einen Gangwar gegen " + factions.Faction.GetPlayerFactionName(_gangwarManager.currentArea.GetCurrentRound().DefenderId) + " gestartet!");
                                                return;
                                            }
                                            else { Reallife.Core.RageAPI.SendChatMessageToAll("!{175,0,0}Das Gebiet " + area.Name + " hat noch einen Cooldown bis zum : " + area.GetLeftTime().ToString()); }
                                        }
                    ...
                    if (factions.Allround.isBadFaction(player) && player.vnxGetElementData<int>(EntityData.PLAYER_FACTION) == _gangwarManager.currentArea.GetCurrentRound().AttackerId)
                    {
                        if (_gangwarManager.currentArea.GetCurrentRound().CurrentState == GangwarRound.RoundStates.PREPARING)
                        {
                                ...
                                || (!GangwarManager.AttackerCountMore && attCount < _gangwarManager.GetFactionCount(_gangwarManager.currentArea.IDOwner)))
                            {
                                ...
                                factions.Faction.CreateCustomFactionInformation(_gangwarManager.currentArea.GetCurrentRound().AttackerId, "!{0,200,0}" +player.Name + " nimmt nun am Gangwar teil.");
                                return;
                        ...
                        if (_gangwarManager.currentArea.GetCurrentRound().CurrentState == GangwarRound.RoundStates.RUNNING)
                        {
                            if (_gangwarManager.currentArea.Position.Distance(player.Position) > GangwarManager.MIN_DIST)
                            {
                                ...
                                if (defCount < allowedDefCount)
                                {
                                    ...
                                    {
                                        _gangwarManager.currentArea.AddPlayer(player);
                                        factions.Faction.CreateCustomFactionInformation(_gangwarManager.currentArea.GetCurrentRound().DefenderId, "!{0,200,0}" +player.Name + " nimmt nun am Gangwar teil.");
                                        factions.Faction.CreateCustomFactionInformation(_gangwarManager.currentArea.GetCurrentRound().AttackerId, "!{150,0,0}" +player.Name + " nimmt nun am Gangwar teil.");
